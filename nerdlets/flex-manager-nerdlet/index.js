...
export default class FlexManager extends React.Component {
  ...
  async fetchLatestInfo() {
    const latest = await fetch(
      'https://api.github.com/repos/newrelic/nri-flex/releases/latest'
  ...
  fetchFlexSummarySamples(accounts) {
    return new Promise((resolve) => {
      const summaryPromises = accounts.map((account) =>
        NerdGraphQuery.query({
          query: gql(q.nrql(account.id, q.nrqlFlexSummary)),
        })
      );
      Promise.all(summaryPromises).then(async (values) => {
        const flexSummarySamples = [];
        const flexGitRepos = {};
        ...
        values.forEach((value) => {
          ...
          const accountId =
            ((((value || {}).data || {}).actor || {}).account || {}).id || null;
          const accountName =
            ((((value || {}).data || {}).actor || {}).account || {}).name ||
            null;
          results.forEach((result) => {
            ...
            const name =
              result['flex.LambdaName'] ||
              result['flex.ContainerId'] ||
              result['flex.Hostname'] ||
              result.hostname ||
              '';

            if (
              result['flex.GitRepo'] &&
              !flexGitRepos[result['flex.GitRepo']].includes(name.toLowerCase())
            ) {
    ...
    return new Promise((resolve) => {
      ...
      summarySamples.forEach((sample) => {
        ...
        if (query && !queries.includes(query)) {
          statusSamplePromise.push(
            NerdGraphQuery.query({
              query: gql(q.nrql(sample.accountId, query)),
            })
          );
          queries.push(query);
        }
  ...
  fetchRepoExamples(links) {
    const repoPromises = links.map((link) =>
      fetch(link).then((response) => response.json())
    );
    Promise.all(repoPromises).then((values) => {
      let exampleRepoConfigsTemp = [];
      values.forEach((value) => {
        exampleRepoConfigsTemp = exampleRepoConfigsTemp.concat(value);
      });
      const exampleRepoConfigs = exampleRepoConfigsTemp.map((val) => {
  ...
  renderMenu(activeItem) {
    return (
      <Menu pointing secondary inverted>
        <Menu.Item
          name="install flex"
          active={activeItem === 'install flex'}
        ...
        <Menu.Item
          ...
          active={activeItem === 'deploy integrations'}
          onClick={this.handleItemClick}
        />
      </Menu>
    );
        ...
        <div style={{ backgroundColor: '#000e0e' }}>
          ...
          >
            ...
            <FlexEntityList
              ...
              activeItem={this.state.activeItem}
              flexStatusSamples={this.state.flexStatusSamples}
              exampleRepoConfigs={this.state.exampleRepoConfigs}
            />
            <FlexRepoList
              handleState={this.handleState}
              activeItem={this.state.activeItem}
