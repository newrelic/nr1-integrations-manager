          ...
          <Table celled inverted>
            ...
            <Table.Body>
              {processes.map((process, i) => {
                const searchWords = `${process.name} ${process.cmd}`
                  .replace(/[^a-zA-Z ]/g, '')
                  .replace(/-/g, ' ')
                  .replace(/_/g, ' ')
                  .split(' ');
                ...
                searchWords.forEach((word) => {
                  this.props.exampleRepoConfigs.forEach((cfg) => {
                    ...
                    cfgSearchWords.forEach((word2) => {
                      if (
                        word.toLowerCase().includes(word2.toLowerCase()) &&
                        !word.includes('newrelic')
                      ) {
                ...
                return (
                  <Table.Row key={i}>
                    ...
                    <Table.Cell>
                      {process.localIP} {process.localPort}
                    </Table.Cell>
                    <Table.Cell>
                      {process.remoteIP} {process.remotePort}
                    </Table.Cell>
                    <Table.Cell>
                      {resultMatches.map((cfg, i) => {
                        return (
                          <div style={{ paddingBottom: '2px' }} key={i}>
                            <Button
                              ...
                              size="mini"
                              onClick={async () => {
                                const header = `### Network Discovery\n`;
                                const networkInfoLocal = `### local ${process.localIP} ${process.localPort}\n`;
                                const networkInfoRemote = `### remote ${process.remoteIP} ${process.remotePort}\n`;
                                const tempConfig = await fetch(
                                  cfg.download_url
